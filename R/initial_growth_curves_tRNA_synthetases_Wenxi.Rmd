...
if (which_try == "first_try") {
  ...
  growth_30 <- read.xlsx(file = paste(base_dir, "TSSC/Data/pilot/pre_test_growth_curves/second_try/20250606-TSGC-30C-48.xlsx", sep=""), sheetIndex = 1)
...
## 0.2. Load and pre-process data - OLD - NEED TO GO THROUGH THIS AND DELETE EVERYTHING THAT IS NOT NECESSARY - BASICALLY FIGURE OUT IF THE WEIRD PLATE WITH THE WRONG EMPTY ROW STILL WORKS WITH MY NEW CODE OR IF I NEED TO KEEP THE PIECES OF THIS WHERE THAT TAKES PLACE
```{r}
...
Create functions to pre-process plate reader growth curves
```{r}
...
Run pre-processing - first try
```{r}
if (which_try == "first_try") {
  # Get the long version, normalized to time 0, averaged across replicates, growth data for each of the temperatures
  ...
  growth_37_avg_norm_long <- process_plate_reader_growth_curves(df = growth_37,
                                                           ...
                                                           long_version = T,
                                                           wrong_plate = F)
  # Join results from this into a single long dataframe to plot from
...
Run pre-processing - second try
```{r}
if (which_try == "second_try") {
  # Get the long version, normalized to time 0, averaged across replicates, growth data for each of the temperatures
  growth_26_avg_norm_long_second <- process_plate_reader_growth_curves(df = growth_26,
                                                           sample_ids = sample_ids, 
                                                           replicates = list(c("A", "B", "C"), c("E", "F", "G")),
                                                           empty_rows = c("D", "H"),
                                                           fixed_temp = 26,
  ...
  growth_34_avg_norm_long <- process_plate_reader_growth_curves(df = growth_34,
                                                           ...
                                                           wrong_plate = F)
  # Join results from this into a single long dataframe to plot from
  growth_long_full_norm <- rbind(rbind(growth_26_avg_norm_long, growth_30_avg_norm_long), growth_34_avg_norm_long)
  
  
  # Get the wide version, without normalization, to check what happened at 30ºC
  ...
  # Get the long version, averaged across replicates but without normalization, growth data for each of the temperatures
  growth_26_avg_long_raw <- process_plate_reader_growth_curves(df = growth_26,
                                                           sample_ids = sample_ids, 
                                                           replicates = list(c("A", "B", "C"), c("E", "F", "G")),
                                                           empty_rows = c("D", "H"),
  ...
  growth_34_avg_long_raw <- process_plate_reader_growth_curves(df = growth_34,
                                                           ...
                                                           wrong_plate = F)
  # Join results from this into a single long dataframe to plot from
  growth_long_full_raw <- rbind(rbind(growth_26_avg_long_raw, growth_30_avg_long_raw), growth_34_avg_long_raw)
}
```
...
# 1. Initial checks
## 1.1. How stable was the temperature along each of the cultures?
```{r}
...
# Be careful that I am not doing this from the original data but from the long version, so if I change anything it might get biased (more observations from one temperature than another or whatever)
...
## 1.2. How consistent is the initial OD across all samples and temperatures - to see if normalizing to it is reasonable
```{r}

```

## 1.3. Check what is wrong with T = 30ºC in the second try of the pilot
```{r}
...
# 2. Main growth curves
## 2.1. With the raw ODs - need to fix the Y axis, otherwise it's different for every plot
WOULD LIKE TO ADD THE INFORMATION TO THE LETHAL ONES OF AT WHICH TEMPERATURE EACH OF THEM IS LETHAL, NEED TO RETRIEVE IT FROM THE LIBRARY FROM LI ET AL., 2015
```{r}
plot_list <- list()
...
## 2.2. With the ODs normalized to time = 0
```{r}
plot_list <- list()
temp <- sample_ids %>%
