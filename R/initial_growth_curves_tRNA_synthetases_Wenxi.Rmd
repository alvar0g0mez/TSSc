...
which_try = "third_try"                 # This indicates which of the 2 sets of OD measurements Wenxi did we want to look at 
...
if (which_try == "first_try") {
  ...
  if (which_try == "third_try") {
    # Second try - data from the second time Wenxi did the growth curves - 48h
    growth_22 <- read.xlsx(file = paste(base_dir, "TSSC/Data/pilot/pre_test_growth_curves/third_try/20250724-highOD-22.xlsx", sep=""), sheetIndex = 1)
    growth_26 <- read.xlsx(file = paste(base_dir, "TSSC/Data/pilot/pre_test_growth_curves/third_try/20250724-highOD-26.xlsx", sep=""), sheetIndex = 1)
    growth_30 <- read.xlsx(file = paste(base_dir, "TSSC/Data/pilot/pre_test_growth_curves/third_try/20250724-highOD-30.xlsx", sep=""), sheetIndex = 1)
    growth_34 <- read.xlsx(file = paste(base_dir, "TSSC/Data/pilot/pre_test_growth_curves/third_try/20250724-highOD-34.xlsx", sep=""), sheetIndex = 1)
}
...
# Create a function which we can use to filter out all data points after a certain time (necessary for the third try, we only want to go up to 6h)
cut_off_at_a_certain_time <- function(df, time_limit) {
  df <- df %>%
